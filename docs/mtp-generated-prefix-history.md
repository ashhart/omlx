...
This path requires `qwen4_exp`, enabled chained MTP priming, a successful full
...
The tests exercise real MTP folds with a tiny random Qwen3.5 model that shares
the fold/cache contract, and check native cache publication, restore, eviction,
terminal cleanup and request isolation. This is not a full Qwen4 model quality
or throughput benchmark. Any end-to-end speed claim needs a repeated-prompt
comparison on the same model, requests and sampling settings.
